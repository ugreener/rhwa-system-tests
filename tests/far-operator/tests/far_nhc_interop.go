	...
	func() {
		...
		JustAfterEach(func() {
			...
			if nhcRemoved && nhcState.fartName != "" {
				By("Cleanup: deleting FARTemplate " + nhcState.fartName)
		...
		It("should remediate unhealthy node when NHC uses FARTemplate",
			reportxml.ID("61309"),
			Label(labels.TierAcceptance),
			func() {
				By("Triggering NHC remediation through a FARTemplate")
		...
		It("should default to reboot when FARTemplate omits action",
			reportxml.ID("66204"),
			Label(labels.TierAcceptance),
			func() {
				By("Triggering NHC remediation through a FARTemplate without an action")
...
func triggerNHCRemediation(
	...
	By("Creating FARTemplate " + fartName)
	...
	Expect(apiClient.Create(ctx, fart)).To(Succeed(),
		"Failed to create FARTemplate %s", fartName)

	nhcName := fmt.Sprintf("nhc-%s", testPrefix)

	By("Creating NHC " + nhcName + " pointing to FARTemplate " + fartName)
